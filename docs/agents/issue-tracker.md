...
- **Commit**: as you go. Every ticket or map edit (claim, comment, resolve, close, new ticket) is committed when made, in its own commit separate from code, with a message naming the ticket — e.g. `Resolve impl ticket 03: lazy services`. The same goes for `CONTEXT.md`, ADRs, and findings under `docs/`: commit them when written, separate from code.
