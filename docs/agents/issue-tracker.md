...
- **Commit**: tickets and maps, as you go. Every ticket or map edit (claim, comment, resolve, close, new ticket) is committed when made, in its own commit separate from code, with a message naming the ticket — e.g. `Resolve impl ticket 03: lazy services`. `CONTEXT.md`, ADRs, and findings under `docs/` are still left as working-tree changes for the user to review and commit.
