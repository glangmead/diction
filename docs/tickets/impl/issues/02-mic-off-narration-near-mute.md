...
### glangmead — 2026-08-24 (reopened after device testing)

`933f41c` did not fix it. On device, tapping the mic repeatedly mid-narration went loud → quiet → near-zero, cumulatively. The simulator has no VPIO path, so a human-in-the-loop trace was built: an instrumented dev build installed with `devicectl`, launched with experiment switches in an env var, stdout streamed back, plus the recognizer's session calls dumped at every transition. Findings, one switch per round:

- **Round 0/1 (current code; then `setVoiceProcessingEnabled(false)` on stop)** — neural voice `L q 0 0 0 0`. The trace showed the third sentence's `AVAudioPlayer` started at :35.9, the mic went off at :39, `setActive(false)` succeeded, and **no further sentence ever started**: the player's finish callback never fired, so `KokoroSpeechEngine.play`'s continuation hung and the pass wedged ("Narrating" forever, silence). The "0" was not attenuation. Explicit VPIO teardown changed nothing.
- **Round 2 (`setActive(false)` skipped, category change only)** — neural `L q L q L q`, Apple `L L L L L L` across six toggles. The Apple-voice near-mute is gone when the `.playback` category change lands on the *active* session, which is what the forum workaround actually does; on an inactive session it does nothing.
- **Round 3 (same, with recognition printed)** — every capture cycle came up `48000 Hz / 1 ch` and "Inventory" was delivered after each toggle and after switching games, so the deactivation's original purpose (avoiding a 0 Hz / 0 ch input format on a fresh engine) is covered by the category flip reconfiguring the I/O.

Also learned: enabling VPIO silently switches the session mode to `.voiceChat`, iOS reports the *call* volume (`outputVolume` 0.55 vs 0.4) while the mic is on, and the "phone-call quality" of mic-on narration is pre-existing (confirmed on the TestFlight build) — out of scope, not a bug. The neural voice being quiet *while* VPIO runs (`q`) is the forum's per-source attenuation and is also pre-existing; out of scope here.

## Answer

_glangmead — 2026-08-24_

Fixed in `933f41c` + `ad3ce27`. The first commit's diagnosis (stale `.playAndRecord` reused for narration) was right about the *state* but wrong about the *cure*: restoring `.playback` after `setActive(false)` does nothing, because the post-VPIO output attenuation is cleared only by a category change on an **active** session — and the deactivation itself wedged the neural voice (see the reopening comment for the device trace).

What the fix is now:

1. `SpeechRecognizer.stopContinuous()` (and a failed `startContinuous`) call `restoreNarrationSession()`: `setCategory` to `ListeningSessionConfig.restoredOnStop` (`.playback` / `.default` / `[.duckOthers]`) on the still-active session. **No `setActive(false)`.** The doc comment records why, and why the old 0 Hz / 0 ch crash concern is covered.
2. `NarrationSessionConfig` is the single narration-session definition with the pure `shouldApply(recognizerLive:currentCategory:)` policy; `SpeechSynthesizer` and the Kokoro audition use it, deciding by `SpeechRecognizer.ownsAudioSession` (via `VoiceCoordinator.isRecognizerLive` and the `\.isRecognizerLive` environment key for the Settings sheet a game presents), never by the session's category.
3. `AudioSessionControlling` — a five-member `@MainActor` protocol `AVAudioSession` conforms to with no glue — is injected into `SpeechRecognizer.init(session:)` so the stop path has a real seam.

Tests: `NarrationSessionConfigTests` (values + `shouldApply` matrix), `ListeningSessionConfigTests.restoredOnStopLeavesPlayAndRecord`, and the regression test `SpeechRecognizerSessionTests.stopRestoresPlaybackWithoutDeactivating` against a recording fake — it goes red on the exact bug (`setActive(false)` on stop) and on a wrong restore. Not written: the ticket's `SpeechSynthesizer`-level test; `speak()` no-ops on the simulator host, so `activatePlaybackSession` is unreachable there even with a session seam, and the decision it makes is the tested `shouldApply`.

Verification: `xcodebuild test` 286 pass, 1 fail (`MisakiPhonemizerTests.problemCorpus()`, pre-existing, see ticket 01); zero warnings; `swiftlint` clean. Device acceptance on iPhone 15 Pro Max, iOS 26: verified by glangmead on the fixed build — accessibility voice and neural voice both stay at full volume across repeated mid-narration mic toggles, neural narration never wedges, mic-off narration and a game switch are full volume, and recognition works after every toggle. Item 8 (AirPods) not reported.

Out of scope, recorded here so nobody re-diagnoses them: mic-on narration has call quality (VPIO puts the session in `.voiceChat` mode; pre-existing on TestFlight, by design of VPIO), and neural-voice sentences started while VPIO runs are attenuated (the forum's per-source attenuation; pre-existing).
